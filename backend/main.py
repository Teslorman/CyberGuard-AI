...
import argparse, json, sqlite3, uuid, shutil, logging, threading, os
...
if os.environ.get('PUBLIC_ORIGIN'):
    allowed_origins.append(os.environ['PUBLIC_ORIGIN'].rstrip('/'))
app.add_middleware(CORSMiddleware,allow_origins=allowed_origins,allow_methods=['GET','POST','PATCH'],allow_headers=['Content-Type'])
...
class RequestGuard:
    ...
    async def __call__(self,scope,receive,send):
        ...
        if scope['method'] not in ('GET','HEAD','OPTIONS') and origin and origin not in allowed_origins:
