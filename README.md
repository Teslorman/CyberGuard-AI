...
The repository includes [`render.yaml`](./render.yaml) for a single Render web service. Create a new Blueprint from the repository in Render and apply the generated `cyberguard-ai` service. The build installs the pinned Python dependencies, Semgrep, the checksum-verified Linux Gitleaks binary, and the Vite frontend; the service then serves both the API and the built UI from the Render port. Set `PUBLIC_ORIGIN` to the deployed service URL if you need cross-origin requests from another frontend origin.

The Blueprint explicitly sets the web service to Render's `free` compute plan and does not use a persistent disk. SQLite data is therefore ephemeral: assessments and findings can be lost when the service restarts, sleeps, or is redeployed. Render may still require a payment method to be added to the account for verification or overage protection; the `$1` authorization is temporary and is not a charge. This application has no authentication and is intended for a trusted single user, so do not publish it without adding an authentication layer.
