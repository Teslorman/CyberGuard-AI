...
The Blueprint is configured for Render's free web service tier and does not use a persistent disk. SQLite data is therefore ephemeral: assessments and findings can be lost when the service restarts, sleeps, or is redeployed. This application has no authentication and is intended for a trusted single user, so do not publish it without adding an authentication layer.
