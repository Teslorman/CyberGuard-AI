...
The Blueprint attaches a 1 GB persistent disk at `data`, because the application stores its SQLite database there. Persistent disks are available only on eligible paid Render plans; without one, assessments and findings are lost when the service is redeployed or restarted. This application has no authentication and is intended for a trusted single user, so do not publish it without adding an authentication layer.
